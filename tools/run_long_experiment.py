#!/usr/bin/env python3
"""Run a long training experiment with decaying parameters.

This script runs a 15,000 episode training where:
...
- Video is recorded at key milestones

Usage:
    cd lunar-lander
    .venv-3.12.5/Scripts/python.exe tools/run_long_experiment.py
"""
...
def main():
    """Run long training experiment with parameter decay."""
    print("=" * 70)
    print("EXP_022: LONG TRAINING WITH DECAYING PARAMETERS")
    print("=" * 70)

    # Experiment paths
    exp_dir = Path(__file__).parent.parent / "experiments" / "EXP_022_exploitation_phases"
    ...
    record_episodes = tuple(
        ...
        list(range(50, 55)) +           # Episodes 50-54
        list(range(100, 103)) +         # Episodes 100-102
        list(range(1000, 1003)) +       # Episodes 1000-1002
        list(range(3000, 3004)) +       # Episodes 3000-3003
        list(range(5000, 5004)) +       # Episodes 5000-5003
        list(range(10000, 10004)) +     # Episodes 10000-10003
        list(range(14996, 15001))       # Episodes 14996-15000
    )
    ...
    run_config = RunConfig(
        ...
        render_mode='none',  # No rendering = lowest memory, fastest training
