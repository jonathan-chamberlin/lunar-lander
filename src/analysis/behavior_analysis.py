...
class BehaviorAnalyzer:
    ...
    def _detect_outcome(
        ...
        # Check for successful landings (both legs on ground, controlled)
        if both_legs and abs(final_angular_vel) < Thresholds.SPINNING:
            ...
            if (abs(final_angle) > Thresholds.SLIGHT_TILT and
                    final_velocity < Thresholds.HARD_LANDING_VELOCITY):
                return "LANDED_TILTED"
            if final_velocity < Thresholds.CRASH_VELOCITY:
                return "LANDED_HARD"
            # High horizontal velocity with both legs down = sliding
            if abs(final_vx) > 0.1 and final_velocity < Thresholds.CRASH_VELOCITY:
                return "LANDED_WITH_DRIFT"

        # Check for single leg landing
        if one_leg and terminated:
            return "TOUCHED_DOWN_ONE_LEG"
    ...
    def _detect_vertical_patterns(
        ...
        if y_variance < Thresholds.LOW_VARIANCE:
            behaviors.append("STABLE_ALTITUDE")
    ...
    def _detect_orientation_patterns(
        ...
        if stats['max_abs_angular_vel'] > Thresholds.SPINNING:
            behaviors.append("EXCEEDED_SPIN_RATE")
